...
consoleioctl(struct file *f, int param, int value)
{
	...
	{
		case SET_FD_COLOR:
			f->dev_payload = value * 0x100;
			break;
		case SET_GLOBAL_COLOR:
			global_color = value * 0x100;
			break;
		default:
...
*/
	void
restore_cursor(struct cursor *cursor)
{
	acquire(&input.lock);

	/* cursor type */
	outb(0x3D4, 0x0A);
	outb(0x3D5, cursor->start);
	outb(0x3D4, 0x0B);
	outb(0x3D5, cursor->end);

	/* cursor pos */
	outb(CRTPORT, 14);
	outb(CRTPORT+1, cursor->pos >> 8);
	outb(CRTPORT, 15);
	outb(CRTPORT+1, cursor->pos);
...
*/
	void
get_cursor(struct cursor *cursor) {

	acquire(&input.lock);

	/* cursor type */
    outb(0x3D4, 0x0A); // Select Cursor Start Register
    cursor->start = inb(0x3D5);
    outb(0x3D4, 0x0B); // Select Cursor End Register
    cursor->end = inb(0x3D5);

	/* cursor pos */
	cursor->pos = 0;
	outb(CRTPORT, 14);
	cursor->pos |= inb(CRTPORT+1) << 8;
	outb(CRTPORT, 15);
